...
- **創業フェーズの CTO 経験**
  - 技術選定・開発・運用から社内 IT まで、事業に必要なものを自分で決めて実装
  - 自社サービスの構想から、Web サイトでの見せ方までを設計
  - 小規模でも維持できる構成の選定（Cloudflare 中心に寄せ、運用コストを抑制）
- **AI システム開発**
  - 人材マッチングや開発案件の自動評価システムを、設計から実運用までフルスタック開発
  - Cloudflare 上に RAG・ベクトル検索・AI エージェントを組み込んだアプリケーションを構築
  - 社内システムを MCP 前提で設計し、非エンジニアが Claude Code / Codex から直接操作
  - AI 判定と人間判断の記録を定期実行で点検し、プロンプトを更新する自己改善ループの構築
- **FDE としての AI 活用支援**
...
- **AI の判断が顧客接点まで届く自動ループの構築（業務3・4）**
  ...
  - AI エージェントの返信・提案には人間の承認と監査記録を必須にし、送信時間帯・連投抑制のガードを設計
...
- **ビジネス指標に基づくグロース施策の企画・実装**
  - ARPU・CTR・LTV などの KPI を CEO と連携して追い、ユーザー獲得コスト対 LTV を意識した施策を設計
  - ファネル分析で離脱ポイントを特定し、課金導線の改善によりサブスク契約者数と課金アイテム購入数を増加
  - ガチャ・ミッション機能・無料開放施策を設計・実装し、主要 KPI を改善
  - オファーウォール連携の導入による収益チャネルの拡大
- **高速な PDCA サイクルの実践**
  - 施策立案から 1 週間でリリースする PDCA を、スプリントごとに反復
  - A/B テストで効果を検証し、季節要因や曜日パターンなど複数の変数がある中で施策単体の効果を測定
  - 効果が見込めない施策は即座に削除し、効果がないという事実も意思決定に反映
- **デザイナーとの協業**
  - 表示内容や画面遷移の導線、コンポーネントの色や配置といった UI/UX の仕様にまで踏み込んだ改善提案
...
- **開発体制の改善活動**
  ...
  - デイリーを毎日開催し、メンバー間の情報共有と開発プロセスの改善を推進
...
- **アジャイル開発**
  - テスタブルなアーキテクチャの導入: モックで API レスポンスを差し替え、API 提供前から実装に着手
...
- **実装・最新技術**
  ...
  - Swift 5 → Swift 6 への移行: 早い段階から Beta 版 Xcode で移行を検証し、大きなトラブルなく完了
...
- チームに経験者のいない AWS Amplify SDK や SwiftProtobuf を、サンプルアプリで先行検証してチームに展開し、採用まで推進
...
- UIKit と Delegate パターンによる既存実装を、SwiftUI・Combine・Swift Concurrency でリファクタリング
- チーム運用の改善として、以下を実施
  - 見積会の実施
  - レトロスペクティブの実施
  - 開発チーム朝ハドル会の実施
...
- iOS リードエンジニアとして、要件の調整とサーバーサイドチームとの API インターフェースの検討を担当
- Scrum Master も兼任
...
- スクラムボードの運用やコードレビューの方法を、レトロスペクティブを待たずにチーム内で相談し改善
...
- BtoC 向けクレジットカードアプリの iOS 開発で、見積もり、実装、テスト、レビュー、バグ修正を担当
- 主担当はサーバーサイドチームで、手が空いたときや iOS チームの負荷が上がったときに iOS を兼任
...
- モバイルアプリのバックエンド開発を、設計・実装・テスト・リリース・運用まで担当
